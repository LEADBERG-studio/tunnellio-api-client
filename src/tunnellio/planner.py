...
from .errors import InteractiveInputRequiredError, ValidationError
from .models import (
    Capabilities,
    ConnectionProfile,
    DomainSummary,
...
class Planner:
        ...
        if key_mode == "new":
            ...
            if not public_key:
                raise ValidationError(
                    ...
                    details={"path": options.public_key_path},
                )
            return {
    ...
    def _validate_key_lifetime(self, lifetime_days: int | None, capabilities: Capabilities) -> None:
        if lifetime_days is None:
            return
        if (
            lifetime_days < capabilities.keys.min_lifetime_days
            or lifetime_days > capabilities.keys.max_lifetime_days
        ):
            raise ValidationError(
    ...
    def _validate_domain_lifetime(self, lifetime_days: int | None, capabilities: Capabilities) -> None:
        if lifetime_days is None:
            return
        if (
            lifetime_days < capabilities.domains.min_lifetime_days
            or lifetime_days > capabilities.domains.max_lifetime_days
        ):
            raise ValidationError(
